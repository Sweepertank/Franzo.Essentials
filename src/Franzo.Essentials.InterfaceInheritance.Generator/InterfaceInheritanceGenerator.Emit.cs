...
public partial class InterfaceInheritanceGenerator : IIncrementalGenerator
{
    internal static void Emit(Context context)
    {
        ...
        Parallel.ForEach(
            context.AnalysisData.RootTypesToEmit,
            (type, _, _) =>
            {
        ...
        void EmitTypes(object? args)
        {
            var castedArgs = (EmitTypesArgs)args!;

            for (var i = castedArgs.StartIndex; i < castedArgs.StopIndex; i++)
            {
                var type = context.AnalysisData.RootTypesToEmit[i];
                ...
                {
                    context.SourceProductionContext.AddSource(
                        $"Franzo.Essentials.InterfaceInheritance.Generated-{i}.cs",
                        s);
                }
            }
        }

        if (CircumventedEnvironment.ProcessorCount < context.AnalysisData.RootTypesToEmit.Count)
        {
            EmitTypes(new EmitTypesArgs(0, context.AnalysisData.RootTypesToEmit.Count));
        }
        else
        {
            var idealTypesPerThread = (int)((float)context.AnalysisData.RootTypesToEmit.Count / CircumventedEnvironment.ProcessorCount);
            var threads = new List<Thread>();
            for (var i = 0; i < CircumventedEnvironment.ProcessorCount; i++)
            {
                var thread = new Thread(EmitTypes);
                var startIndex = i * idealTypesPerThread;
                var stopIndex = startIndex + idealTypesPerThread;
                if (i == CircumventedEnvironment.ProcessorCount - 1)
                {
                    stopIndex = context.AnalysisData.RootTypesToEmit.Count;
                }

                thread.Start(new EmitTypesArgs(startIndex, stopIndex));
                threads.Add(thread);
            }
        ...
        /*foreach (var type in context.AnalysisData.RootTypesToEmit)
        {
            ...
            context.SourceProductionContext.AddSource(
                $"Franzo.Essentials.InterfaceInheritance.Generated-{count}.cs",
                sb.ToString());
            count++;
        }*/
    }
